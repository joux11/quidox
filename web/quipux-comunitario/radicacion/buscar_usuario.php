...
<?
		$sql = "select lista_nombre, lista_codi from lista where 
        (usua_codi=0 and inst_codi=?) or (usua_codi=? and lista_estado = 1) order by 1 asc";
		$rs = $db->conn->prepare($sql);
		$rs->execute([$_SESSION["inst_codi"], $_SESSION["usua_codi"]]);
...
if (($buscar_nom!="" or $buscar_car!="" or $buscar_inst!="0") and $lista_usr=="0") {
    ...
    $params = [$_SESSION["inst_codi"], $buscar_tipo];
    ...
    $sql .= " and upper(usua_nombre) not like '%ADMIN%' order by usua_nombre asc";

    $rs = $db->conn->prepare($sql);
    $resultado_rs=$rs->execute($params);
//echo $sql;
}
if ($lista_usr!="0") {
	$sql = "select u.* from usuario u, lista_usuarios as l
            where u.usua_codi=l.usua_codi and lista_codi=? and upper(usua_nombre) not like '%ADMIN%' order by usua_nombre asc";
    $rs = $db->conn->prepare($sql);
    $resultado_rs=$rs->execute([$lista_usr]);
}
//echo 'SQL: >' . $sql . "<";
if ($sql!="") {
  	$rs=$resultado_rs; 
