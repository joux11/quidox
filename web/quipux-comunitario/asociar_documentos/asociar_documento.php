...
**/
  ...
  if (strpos($_SERVER["HTTP_REFERER"],"asociar_documento.php")===false) { 
        ...
        $sql1 = "SELECT radi_nume_asoc FROM radicado WHERE radi_nume_radi = ?";
        $stmt1 = $db->conn->Prepare($sql1);
        $rs = $db->conn->Execute($stmt1, array($radi_nume));
        $txt_radi_asoc_ante = $rs && !$rs->EOF ? $rs->fields["RADI_NUME_ASOC"] : "";

        $sql2 = "SELECT radi_nume_radi FROM radicado WHERE radi_nume_asoc = ?";
        $stmt2 = $db->conn->Prepare($sql2);
        $rs = $db->conn->Execute($stmt2, array($radi_nume));
        $txt_radi_asoc_cons = "";
