...
switch ($db->driver) {
    case 'postgres':
        ...
        $from_usr_recorrido = " radi_nume_radi in (
            select distinct radi_nume_radi from hist_eventos 
            where usua_codi_ori = ? or usua_codi_dest = ?
        )";
        ...
        if ($editar == 1) {
            $isql .= ",
                'Antecedente' AS \"SCR_Acción\",
                'seleccionar_documento(\"'|| radi_nume_radi ||'\",\"A\");' as \"HID_FUNCIONA\"";
        }

        $isql .= ",
            case when radi_nume_asoc is null then 'Consecuente' else '' end AS \"SCR_Acción.\",
        ...
        from (
            ...
            from (
                select * from radicado b where 
                    UPPER(radi_nume_text || ' ' || coalesce(radi_cuentai, '')) like ?
                    and radi_inst_actu = ?
                    and esta_codi in (0,1,2,3,4,5,6)
                    and radi_nume_radi <> ?
            ) as r
        ...
        $txt_documento = isset($txt_documento) ? trim($txt_documento) : '';
        $txt_documento_like = '%' . strtoupper($txt_documento) . '%';

        $stmt = $db->conn->Prepare($isql);
        $rs = $db->conn->Execute($stmt, array($usuarioSel, $usuarioSel, $txt_documento_like, $_SESSION["inst_codi"], $radi_nume));

        break;
}
// FIN security/sed-8-inyection
