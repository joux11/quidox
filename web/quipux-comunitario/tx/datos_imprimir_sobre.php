...
if($tipoUsuario==1) //Obtener ID de ciudad para funcionario
{
    ...
    $sqlCiu = "SELECT ciu_codi FROM usuarios WHERE usua_codi = ?";
    $params = array($_GET['txt_usua_codi']);
    $rsCiu = $db->conn->Execute($sqlCiu, $params);
    ...
    {
        $sqlCiu = "SELECT ciudad_codi FROM ciudadano WHERE ciu_codigo = ?";
        $params = array($_GET['txt_usua_codi']);
        $rsCiu = $db->conn->Execute($sqlCiu, $params);
