...
// Paso 2: validar y limpiar cada valor (ejemplo, si son números enteros)
foreach ($radicadosArray as &$valor) {
    $valor = trim($valor);
    // si no son números, usar validación o lanzar error
    if (!ctype_digit($valor)) {
        // Aquí puedes manejar error o limpiar de otra forma
        die("Error: radicado inválido");
    }
}
unset($valor); // para no modificar accidentalmente después

// Paso 3: crear placeholders
$placeholders = implode(',', array_fill(0, count($radicadosArray), '?'));


$radicadosSel = array();

// Paso 4: preparar consulta parametrizable
$sql = "select radi_nume_radi from radicado where radi_nume_radi in ($placeholders)";

// Ejecutar pasando el array con los valores
