...
if (trim($radicado["fecha_firma"]) == "") {
    ...
    $sql = "select radi_nume_radi
            from radicado
            where radi_nume_temp = ?
                and (radi_nume_radi = ? or replace(radi_usua_dest,'-','')::integer in (select usua_codi from usuarios))";

    $rs = $db->conn->Execute($sql, array($radi_nume, $radi_nume));
    while (!$rs->EOF) {
