...
if($radi_temp) {	/////////////	SI YA EXISTIAN DOCUMENTOS	////////////////////
	...
	// INICIO security/sed-3-sql-inyection A QUERIES PARAMETRIZABLES
	$sql = "SELECT anex_codigo, anex_nombre, anex_desc, anex_path AS archivo,
               anex_tamano, usua_nombre, anex_fecha, anex_fisico
        FROM anexos
        LEFT OUTER JOIN usuario ON anex_usua_codi = usua_codi
        WHERE anex_radi_nume = ? AND anex_borrado = 'N'
        ORDER BY anex_codigo";

	$stmt = $db->conn->Prepare($sql);
	$rs = $db->conn->Execute($stmt, array($radi_temp));

	if ($rs->EOF) {
		die ("<tr><td class='listados1' align='center' colspan='7'>El documento no tiene archivos anexos.</td></tr></table><br>");
	}
	// FIN security/sed-3-sql-inyection A QUERIES PARAMETRIZABLES
