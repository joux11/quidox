...
**/
    ...
    // INICIO security/sed-5-sql-inyection A QUERIES PARAMETRIZABLES
    if (!empty($_GET["codDepe"])) {
        ...
        $stmt = $db->conn->Prepare($sqlDepeHijo);
        $rsDepeHijo = $db->conn->Execute($stmt, array($_GET["codDepe"]));
    }


    if ($rsDepeHijo && !$rsDepeHijo->EOF) {
    ...
    // FIN security/sed-5-sql-inyection A QUERIES PARAMETRIZABLES
