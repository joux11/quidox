...
// Verificamos si es usuario con consulta segura
$sql = "SELECT usua_cedula, usua_email, tipo_usuario FROM usuario 
        WHERE usua_esta = 1 AND usua_codi <> 0 AND usua_login LIKE upper(?) 
        ORDER BY tipo_usuario ASC";
$rs = $db->conn->Execute($sql, ["U$txt_login"]);

if ($rs && !$rs->EOF) {
    // No mostramos la cédula ni email por defecto
    $txt_cedula = "";
    $txt_email  = "";
    $txt_tipo_usr = "<input type='hidden' id='txt_tipo_usr' name='txt_tipo_usr' value='" . htmlspecialchars($rs->fields["TIPO_USUARIO"], ENT_QUOTES, 'UTF-8') . "'>";
    ...
    if ($rs->fields["TIPO_USUARIO"] == 2 && 
        (substr($rs->fields["USUA_CEDULA"], 0, 2) == "99" || trim($rs->fields["USUA_EMAIL"]) == "")) {
...
// Escape del login también
$txt_login_safe = htmlspecialchars($txt_login, ENT_QUOTES, 'UTF-8');

$txt = "<center>
            <br>$txt $txt_cedula $txt_email $txt_tipo_usr
            <input type='hidden' id='txt_login' name='txt_login' value='$txt_login_safe'>
