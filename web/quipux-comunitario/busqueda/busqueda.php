...
// INICIO Bloque de código modificado para la security/sde-1-sql-inyection
if ($txt_depe_codi==$_SESSION["depe_codi"]) 
    $span_depe_nombre = $_SESSION["depe_nomb"];
elseif ($txt_depe_codi == 0) 
    $span_depe_nombre = "&lt;&lt Todas las &aacute;reas &gt;&gt;";
else {
    $sql = "SELECT depe_nomb FROM dependencia WHERE depe_codi = ?";
    $stmt = $db->conn->Prepare($sql);
    $rs = $db->conn->Execute($stmt, array($txt_depe_codi));
    
    if ($rs && !$rs->EOF) {
        $span_depe_nombre = $rs->fields["DEPE_NOMB"];
    } else {
        $span_depe_nombre = "Área no encontrada";
    }
}
// FIN Bloque de código modificado para la security/sde-1-sql-inyection
...
if ($txt_usua_codi == 0) {
    $span_usua_nombre = "&lt;&lt Todos los usuarios &gt;&gt;";
} else {
    $sql = "SELECT usua_apellido, usua_nomb FROM usuarios WHERE usua_codi = ?";
    $stmt = $db->conn->Prepare($sql);
    $rs = $db->conn->Execute($stmt, array($txt_usua_codi));

    if ($rs && !$rs->EOF) {
        $span_usua_nombre = $rs->fields["USUA_APELLIDO"] . " " . $rs->fields["USUA_NOMB"];
    } else {
        $span_usua_nombre = "Usuario no encontrado";
    }
}
